...
def _normalize_eventbus_suffix(raw_suffix: str) -> str:
	"""Normalize an identifier suffix so that ``Agent_{suffix}`` is a valid identifier."""

	suffix = raw_suffix.replace('-', '_')
	suffix = re.sub(r'[^0-9A-Za-z_]', '', suffix)
	...
	if not f'Agent_{suffix}'.isidentifier():
		suffix = 'agent'
...
class Agent(Generic[Context, AgentStructuredOutput]):
	...
	def __init__(
		...
		**kwargs,
		...
		# wal_path = CONFIG.BROWSER_USE_CONFIG_DIR / 'events' / f'{self.session_id}.jsonl'
                self.eventbus = EventBus(name=f'Agent_{_normalize_eventbus_suffix(str(self.id)[-4:])}')
	...
	def _setup_action_models(self) -> None:
        ...
        def add_new_task(self, new_task: str) -> None:
                ...
                agent_id_suffix = _normalize_eventbus_suffix(str(self.id)[-4:])
